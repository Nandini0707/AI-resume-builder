...
const ResumeBuilder = () => {
  ...
  const handleShare=()=>{
     ...
     if(navigator.share){
      ...
      alert('Share not supported on tis browser.')
     }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8 ">
                 ...
                 <div className="flex items-center">
                   {
                    activeSectionIndex!== 0 && (
                      <button onClick={()=>setActiveSectionIndex((prevIndex)=> 
                        Math.max(prevIndex-1,0))} className="flex items-center
                        gap-1 p-3 rounded-lg text-sm font-medium text-gray-600
                        hover:bg-gray-50 transition-all"  disabled={activeSectionIndex ===0}>
                        <ChevronLeft className="size-4"/> Previous
                        </button>
                    )}
                     <button onClick={()=>setActiveSectionIndex((prevIndex)=> 
                        Math.min(prevIndex+1,sections.length-1))} className={`flex items-center
                        gap-1 p-3 rounded-lg text-sm font-medium text-gray-600
        ...
        <button onClick={()=>{toast.promise(saveResume,{loading:"Saving..."})}} className="bg-gradient-to-br from-green-100 to-green-200 ring-green-300 text-green-600 ring hover:ring-green-400 transition-all rounded-md px-6 py-2 mt-6 text-sm">
         ...
         <div className="lg:col-span-7 mx-lg:mt-6">
          ...
          <div ref={printRef} className="resume-print bg-white">
  <ResumePreview
    data={resumeData}
