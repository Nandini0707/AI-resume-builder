...
const PersonalinfoForm = ({ data,onChange,removeBackground,setRemoveBackground}) => {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-600"> Get started with your personal 
        information</p>
      ...
      <div className="flex items-center gap-2">
        <label className="cursor-pointer">
          {data.image ? (
            <img src={ typeof data.image === "string"? data.image: URL.
              createObjectURL(data.image) } alt="user" className="w-16 h-16
               rounded-full object-cover ring ring-slate-300 hover:opacity-80" />
          ) : (
        ...
        {typeof data.image === "object" && (
          <div className="flex flex-col gap-1 pl-4 text-sm">
            ...
            text-gray-900 gap-3">
              <input type="checkbox" className="sr-only peer" checked={removeBackground}
        ...
        return (
          <div key={field.key} className="space-y-1 mt-5">
            ...
            <input type={field.type} value={data[field.key] || ""}
              ...
              transitions-colors text-sm" placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required} />
          </div>
        )
      })}
    </div>
